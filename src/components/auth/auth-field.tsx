  ...
  ({ label, error, hint, icon, className, id, ...props }, ref) => {
    ...
    return (
      <div className="space-y-1.5">
        <label htmlFor={fieldId} className="font-mono text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">
          {label}
        </label>
        <div className="relative">
          ...
          <input
            ...
            id={fieldId}
            className={cn(
              "w-full rounded-xl border border-border bg-surface px-4 py-3 text-sm transition-default placeholder:text-muted-foreground/60 focus:border-primary/40 focus:outline-none focus:ring-2 focus:ring-primary/20",
              icon && "pl-10",
