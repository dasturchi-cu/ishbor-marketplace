...
const actionPrimary =
  "bg-primary text-primary-foreground shadow-[0_8px_20px_-8px_oklch(0.546_0.185_257/0.35)] hover:opacity-90";

export function FreelancerCard({ f }: { f: Freelancer }) {
  return (
    <div className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-border bg-card transition-default hover-lift hover:border-primary/20 hover:shadow-[0_20px_56px_-16px_oklch(0.546_0.185_257/0.14)]">
      <SaveButton
        type="freelancer"
        id={f.username}
      ...
      <Link to="/freelancers/$username" params={{ username: f.username }} className="block flex-1 p-5 pb-4 pr-14">
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
          <LevelBadge level={f.level} className="!px-2 !py-0.5 !text-[9px]" />
          {f.identityVerified && <VerifiedIdentityBadge className="!px-2 !py-0.5 !text-[9px]" />}
        </div>

        <div className="mt-4 flex items-center justify-between gap-3 border-t border-border/70 pt-4">
          <div className="flex flex-wrap items-center gap-2">
            <span className="inline-flex items-center rounded-md border border-border bg-surface px-2 py-1 font-mono text-[10px] text-muted-foreground">
              <span className="font-semibold text-foreground">{f.successScore}</span>
              <span className="ml-1">ball</span>
            </span>
            <span className="inline-flex items-center rounded-md border border-border bg-surface px-2 py-1 font-mono text-[10px] text-muted-foreground">
              <span className="font-semibold text-foreground">{f.completionRate}%</span>
              <span className="ml-1">bajarilgan</span>
            </span>
          </div>
      ...
      <Link to="/services/$slug" params={{ slug: s.slug }} className="block flex-1">
        ...
        >
          ...
          <div className="absolute inset-x-0 bottom-0 flex justify-end p-3">
            <div className="rounded-full border border-white/25 bg-white/95 px-3 py-1.5 shadow-sm backdrop-blur-sm">
