      ...
      {expanded && (
        ...
        >
          <button
            type="button"
            onClick={() => setExpanded(false)}
            className="absolute right-4 top-4 inline-flex size-10 items-center justify-center rounded-lg bg-white/10 text-white focus-ring"
          ...
          >
            <div
              className="grain aspect-[16/9] w-full"
              style={{
                background: `linear-gradient(135deg, oklch(0.72 0.15 ${images[active]!.hue}) 0%, oklch(0.42 0.12 ${images[active]!.hue + 30}) 100%)`,
              }}
            />
            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/60 to-transparent p-6">
              <div className="text-lg font-medium text-white">{images[active]!.caption}</div>
            </div>
          </div>
        </div>
      )}
