...
type VideoIntroProps = {
  ...
  fileUrl?: string;
  posterUrl?: string;
  /** Mock demo — gradient placeholder */
  isDemo?: boolean;
};
...
function VideoPlayer({
  ...
  const src = fileUrl ?? (url && /\.(mp4|webm|mov)(\?|$)/i.test(url) ? url : undefined);
  if (src) {
    return (
      <video
        src={src}
...
export function VideoIntro({
  ...
  const hasRealMedia = !isDemo && !!(url || fileUrl);
  const showDemo = isDemo || !hasRealMedia;
  ...
  return (
    <>
      <div className="flex h-full flex-col">
        ...
        >
          {hasRealMedia && posterUrl ? (
            <div className="relative aspect-video w-full overflow-hidden lg:min-h-[220px] lg:aspect-auto lg:h-full">
              <img src={posterUrl} alt="" className="h-full w-full object-cover" />
              <div className="absolute inset-0 flex items-center justify-center bg-black/25 transition-default group-hover:bg-black/35">
                <div className="inline-flex size-14 items-center justify-center rounded-full bg-white/95 shadow-lg transition-transform group-hover:scale-105 sm:size-16">
          ...
          <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 via-black/30 to-transparent p-4 sm:p-5">
            <div className="font-mono text-[10px] uppercase tracking-[0.16em] text-white/70">Video tanishtiruv</div>
            <div className="mt-0.5 text-sm font-semibold text-white sm:text-base">
              {name} o&apos;z ishini tanishtirmoqda
            </div>
          </div>
          {duration && duration !== "—" && (
            <span className="absolute right-4 top-4 rounded-full bg-black/60 px-2.5 py-1 font-mono text-[10px] text-white">
              {duration}
            </span>
          )}
          {showDemo && (
        ...
        <p className="px-5 py-4 text-xs leading-relaxed text-muted-foreground sm:px-6">
          {name.split(" ")[0]} bilan bevosita bog&apos;laning — jarayoni, so&apos;nggi loyihalari va hamkorlikda nimalarni kutish
          mumkinligi haqida.
        </p>
      </div>
      ...
      <Modal
        ...
        title="Video tanishtiruv"
        description={`${name}${duration && duration !== "—" ? ` · ${duration}` : ""}`}
      >
        {hasRealMedia ? (
          <div className="aspect-video w-full overflow-hidden rounded-xl bg-black">
            <VideoPlayer url={url} fileUrl={fileUrl} posterUrl={posterUrl} className="aspect-video" />
          </div>
        ) : (
          <div
          ...
          >
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 p-6 text-center text-white">
              ...
              <div className="font-mono text-xs uppercase tracking-widest opacity-70">
                Demo tanishtiruv {duration !== "—" ? `· ${duration}` : ""}
              </div>
            </div>
          </div>
        )}
