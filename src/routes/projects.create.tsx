...
function CreateProjectPage() {
  ...
  const handleSaveDraft = () => {
    ...
    actionFeedback.draftSaved("Loyiha qoralamasi", {
      listHint: "Mening loyihalarim → Qoralamalar",
      onViewList: () => navigate({ to: "/my-projects", search: { tab: "draft" } }),
    });
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6">
        ...
        {isDraftEdit && (
          <div className="mt-5 flex flex-col gap-3 rounded-xl border border-primary/20 bg-primary/5 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-start gap-3">
              <div className="grid size-9 shrink-0 place-items-center rounded-lg bg-primary/10 text-primary">
                <FileText className="size-4" />
              </div>
              <div>
                <p className="text-sm font-semibold">Qoralama saqlangan</p>
                <p className="mt-0.5 text-xs text-muted-foreground">
                  Brauzeringizda saqlangan. Keyinroq{" "}
                  <Link to="/my-projects" search={{ tab: "draft" }} className="font-medium text-primary hover:underline">
                    Mening loyihalarim → Qoralamalar
                  </Link>{" "}
                  bo&apos;limidan davom eting.
                </p>
              </div>
            </div>
            <Link
              to="/my-projects"
              search={{ tab: "draft" }}
              className="inline-flex shrink-0 items-center justify-center rounded-lg border border-primary/25 bg-card px-3 py-2 text-xs font-semibold text-primary hover:border-primary/40"
            >
