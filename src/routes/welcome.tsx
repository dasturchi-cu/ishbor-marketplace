...
export const Route = createFileRoute("/welcome")({
  validateSearch: (search: Record<string, unknown>) => ({
    setup: search.setup === "complete" ? ("complete" as const) : undefined,
  }),
  head: () => ({
          ...
          <div className="rounded-2xl border border-border bg-card p-8 shadow-[0_16px_48px_-24px_oklch(0.546_0.185_257/0.18)] sm:p-10">
            ...
            <div className="text-center">
              <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-primary">
                {isComplete ? "Setup complete" : "You're verified"}
              </p>
              <h1 className="font-display mt-2 text-3xl font-extrabold tracking-tight text-foreground sm:text-4xl">
                {isComplete ? `You're all set, ${firstName}` : `Welcome, ${firstName}`}
              </h1>
              <p className="mt-3 text-sm leading-relaxed text-muted-foreground">
                {isComplete
                  ? `Your ${state.userType} profile is live. Start ${state.userType === "client" ? "hiring vetted talent" : "receiving project invites"} with full escrow protection.`
                  : `Your account is ready. Complete a quick ${steps.length}-step setup to start ${state.userType === "client" ? "hiring talent" : "getting hired"} on Ishbor.`}
              </p>
            </div>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center">
              {isComplete ? (
                <>
                  ...
                  >
                    {state.userType === "client" ? "Browse talent" : "Find projects"}
                  </Link>
                </>
              ) : (
                <>
          ...
          {!isComplete && (
            <div className="mt-6 rounded-2xl border border-border bg-card/60 p-5 backdrop-blur-sm">
              <div className="font-mono mb-4 text-center text-[9px] uppercase tracking-widest text-muted-foreground">
                What&apos;s ahead — {steps.length} steps · ~3 min
              </div>
              ...
              {steps.length > 3 && (
                <p className="mt-3 text-center text-xs text-muted-foreground">
                  +{steps.length - 3} more after that
                </p>
              )}
            </div>
          )}
